...
def predict(text, image, checkpoint_path, sensitivity):
    ...
    if clean_input_text in demo_texts:
        ...
        for c in cfg.LABEL2ID.keys():
            if c == target_class:
                demo_class_probs[c] = 0.98
            else:
                demo_class_probs[c] = 0.005
        ...
        demo_final_text = " ".join(highlighted_text)
        
        return clean_text, pil_image, demo_final_text, heatmap_img, masked_img, demo_class_probs, "Demo Match Loaded Successfully"
    # --- END DEMO BYPASS ---
